  ...
  return (
    <div className="border-2 border-iron-200 rounded-lg hover:border-primary/30 transition-colors overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {/* Mini-Übersicht der Werte */}
          <div className="hidden md:flex items-center gap-1 text-xs">
            {Object.entries(candidate.dimensions).slice(0, 4).map(([dim, val]) => (
              <span 
                key={dim} 
                className={`px-1.5 py-0.5 rounded ${
                  val <= 3 ? 'bg-red-100 text-red-700' : 
                  val <= 6 ? 'bg-green-100 text-green-700' : 
...
function InterpretationContent() {
  ...
  const handleAddCandidate = () => {
    ...
    setNewCandidateName('');
    // Neuen Kandidaten automatisch expandieren
    setTimeout(() => {
      const newId = sessionData.candidates[sessionData.candidates.length - 1]?.id;
      if (newId) {
        setExpandedCandidates(prev => new Set([...prev, newId]));
      }
    }, 100);
  };
...
- Deutschsprachig`;
  ...
  const handleSendMessage = async (message) => {
    ...
    try {
      ...
      updateSession({ interpretationChat: [...updatedChat, assistantMessage] });
    } catch (error) {
      ...
      updateSession({ interpretationChat: [...updatedChat, errorMessage] });
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  ...
                  {sessionData.isStandardProcess && (
                    <span className="ml-2 text-primary">(Standardprozess 2/3)</span>
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-iron-200 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <ChatInterface
                ...
                isLoading={isLoading}
                placeholder={sessionData.candidates.length === 0 
                  ? "Bitte fügen Sie zuerst Kandidaten hinzu..." 
                  : "Stellen Sie Fragen zur Interpretation der Ergebnisse..."
                }
